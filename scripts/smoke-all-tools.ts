 ...
 *
 *   tsx scripts/smoke-all-tools.ts
 */
...
async function main() {
  ...
  if (createResult.ok) {
    ...
    // ── draft_message ──────────────────────────────────────────────────────────
    console.log("\ndraft_message");
    // Should fail (no email channel would mean no channel — but we added one above)
  ...
  if (remResult.ok) {
    ...
    console.log("\ncancel_reminder");
    ok("cancel", await executeTool("cancel_reminder", { reminder_id: rd.id }, ctx));
