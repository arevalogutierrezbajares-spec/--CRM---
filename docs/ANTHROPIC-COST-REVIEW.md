...
- Calls without explicit spend context still enforce the same daily cap (workspace-wide when workspace is unknown) before making the API call.
...
- Route-level spend metadata now records route/user/workspace for all active call sites with `trackUsage: true`.
