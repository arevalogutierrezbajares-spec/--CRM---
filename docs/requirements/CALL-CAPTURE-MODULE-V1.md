...
- FR-CALL-TRG-2 (MUST): Capture begins only after the founder affirms the prompt or starts manually. The prompt persists while the call is live (pre-roll rolling in RAM) and ends to NOT recording when the call ends unanswered, the founder dismisses it, or an absolute safety cap fires (`maxRecordingSeconds`; 10 min where call-end is unobservable). An explicit dismissal suppresses re-prompting until the mic is released, so the same call never re-asks. Acceptance: never answering the prompt results in no recording; dismissing it mid-call produces no second prompt for that call. *(Revised 2026-06-12 — the original fixed 60 s expiry re-prompted in a loop that wiped pre-roll each cycle and lost the first 8.5 minutes of a real call.)*
- FR-CALL-TRG-3 (MUST): Audio heard between call start and the founder's affirmation is included in the recording (pre-roll, up to the 60 s ring capacity), so saying yes 20 seconds in loses nothing. Pre-roll of declined or expired prompts is discarded unrecoverably. Acceptance: affirm at t+20s → transcript includes t0 greeting; decline → no audio persisted.
